                  ...
                  if (!string.IsNullOrEmpty(user))  {
                      ...
                      string[] rols = (ds.Tables[0].Rows[0]["Setting"].ToString()).Split(',') ;
                         foreach (string rol in rols ){
                             ...
                             {
                                 string sqltitle = "SELECT title,url FROM  OA_Url WHERE rol LIKE '%" + rol + "%' and Rol NOT IN ('learning-edit','placard-edit','task-edit','user-edit','user-edit','grade-edit','grade-add','student-add','student-edit','pm-add','pm-edit','mech-add','mech-edit') ";
